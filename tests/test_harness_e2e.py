...
any attempt to open it raises, so the test itself proves the harness never reads it.
Synthetic labels are random (seeded), not derived from any human label. Everything is
written to a temporary directory. Embeddings replay offline from the cache. Exit code 1 on
any failure.
"""
import builtins
import io
...
labels_state_before = REAL_LABELS.stat().st_mtime_ns if REAL_LABELS.exists() else None
...
def fenced_open(file, *args, **kwargs):
    try:
        if Path(file).resolve() == REAL_LABELS:
...
def mock_agent(messages):
    ...
    return json.dumps({"intent": intent, "intent_confidence": 0.5 + (len(msg) % 50) / 100, "needs_human": "refund" in msg,
                       "escalation_reason": "compensation_dispute" if "refund" in msg else None,
                       "reply": "We're sorry to hear this. Please DM us and we'll look into it for you."})
...
try:
    ...
    check(all((run_dir / f"{s}.jsonl").exists() for s in RS.ALL_SYSTEMS) and (run_dir / "run_info.json").exists(),
          "one predictions file per system plus run_info.json")
    ...
    check(all(0 <= r["consistency"] <= 1 for r in a_recs), "self-consistency confidence recorded for the agent")
    ...
    base = sum(json.loads(l)["escalate"] for l in open(labels_path, encoding="utf-8")
               if json.loads(l)["case_id"] in set(uniform_ids)) / 130
    ...
    check(hl["A-no-guard"]["escalation"]["coverage"] >= hl["A"]["escalation"]["coverage"],
          "the guard only removes coverage, never adds it")
    ...
    b = hl["A"]["risk_coverage"]["best_under_bar"]
    check(b["threshold"] == "all_escalated" or b["unsafe_auto_rate"] <= 0.05, f"operating point respects the 5% bar: {b}")
    check(hl["A"]["risk_coverage"]["confidence_source"] == "consistency", "risk-coverage uses self-consistency when present")
    rc = {s: hl[s]["reply_checks"]["all_replies"] for s in hl}
    check(rc["A"]["passes"] == 1.0 and rc["B1"]["fabricated"] > 0,
          f"reply checks: the clean mock reply passes; verbatim B1 replies carry invented specifics ({rc['B1']['fabricated']:.0%})")
    check(set(res["comparisons_headline"]) == {"A vs B1", "A vs B0-auto-all"}, "paired comparisons on the headline set")
    ...
    check(md.startswith("# Evaluation results") and "SMOKE TEST" in md and "Headline: 130 uniform" in md,
          "results.md is marked as a smoke test and leads with the headline set")
    ...
    if not ok:
        try:
            ...
            check("refusing" in str(e) and not (tmp / "nomodel").exists(),
                  f"the agent refuses to run without its model ({why[:50]})")
finally:
    llm.clear_overrides()
    builtins.open, io.open = _open, _io_open
    shutil.rmtree(tmp, ignore_errors=True)

after = REAL_LABELS.stat().st_mtime_ns if REAL_LABELS.exists() else None
check(after == labels_state_before, "the real labels file was never opened or changed")
