...
This is the full log; [DECISIONS.md](DECISIONS.md) condenses it to the 15 main decisions.
Format: **decision**, then why.
...
27. **No judge score is used as evidence of reply quality, and the re-label round was skipped.**
    On the 50 test items, both local judges agree with the human at chance level on every
    dimension: quadratic κ from −0.19 to 0.15, and sendable κ of 0.04 (llama3.2:3b) and 0.05
    (qwen2.5:3b). Both are harsher than the human and call almost nothing sendable. Reply quality
    therefore rests on the human scores and the deterministic checks. The rubric was not reworded
    on the 30 development items: a 3B judge at κ ≈ 0 needs a stronger model, not new wording, and
    chasing agreement on 80 items would be tuning on the test. The harness stays unchanged, ready
    for a stronger judge against the same human scores. The 30-case blind re-label
    (`src/relabel.py`) needs a gap of at least 24 hours after the first labels, which the
    submission deadline didn't allow. It was not run, and intra-annotator agreement is reported
    as not measured.
